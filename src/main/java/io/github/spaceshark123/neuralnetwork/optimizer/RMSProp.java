	...
	public void step(double[][] avgBiasGradient, double[][][] avgWeightGradient, double learningRate) {
		for (int i = 1; i < neuronsPerLayer.length; i++) {
			for (int j = 0; j < neuronsPerLayer[i]; j++) {
				// update cache
				biasCache[i][j] = decayRate * biasCache[i][j]
						+ (1 - decayRate) * avgBiasGradient[i][j] * avgBiasGradient[i][j];
				// apply update
				biases[i][j] = biases[i][j]
						- learningRate * avgBiasGradient[i][j] / (Math.sqrt(biasCache[i][j]) + epsilon);
				for (int k = 0; k < neuronsPerLayer[i - 1]; k++) {
					// update cache
					weightCache[i][j][k] = decayRate * weightCache[i][j][k]
							+ (1 - decayRate) * avgWeightGradient[i][j][k] * avgWeightGradient[i][j][k];
					// apply update
